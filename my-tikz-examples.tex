...
\centering 
\begin{tikzpicture}
\node[left] at (0,0) {$ \displaystyle \frac{A_{N+1}}{(1+r)^{t}}$};
\node[right] at (3,2) {$<0$}; 
\node[right] at (3,0) {$=0$}; 
\node[right] at (3,-2) {$>0$}; 

\node[right, text width = 3cm, text centered] at (4,2) {Muere endeudado, esquema de ponzi}; 
\node[right, text width = 3cm, text centered] at (4,0) {Planificación en todo su horizonte de vida}; 
\node[right, text width = 3cm, text centered] at (4,-2) {No optimiza, deja riqueza}; 
...
\draw[black, line width=1.5pt, domain=-7:7,samples=200] plot (\x,{-0.04*(\x)^(2)+1}) node [ right ] {$y$};
